...
def question_and_choices(voting_context, question_id):
        ...
        if not voting_context.request.user.is_anonymous():
            for choice in choices:
                if question.multipleAnswersAllowed:
                    choice_html += '<label class="checkbox-inline mt-4 mr-4"><input type="checkbox" class="mr-2" name="answer_%d" value="%d_%d" %s>%s</label>' % (question.id, question.id, choice.id, 'checked="checked"' if choice in [answer.choice for answer in current_answers] else '', choice.text)
                else:
                    choice_html += '<label class="radio-inline mt-4 mr-4"><input type="radio" class="mr-2" name="answer_%d" value="%d_%d" %s>%s</label>' % (question.id, question.id, choice.id, 'checked="checked"' if choice in [answer.choice for answer in current_answers] else '', choice.text)
        else:
    ...
    # voting complete
        ...
        if len(current_answers) > 0:
            choice_html += '<label class="mt-4 mr-4">(You voted %s)</label>' % comma_and([answer.choice.text for answer in current_answers])
